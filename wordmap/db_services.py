...
def user_input():
    ...
    if request.method == 'POST':
        ...
        sql = "INSERT INTO message_database (source, message) VALUES (%s, %s)"
...
def db_service():
    ...
    summary_query = ("SELECT COUNT(DISTINCT source) AS total_unique_sources, COUNT(DISTINCT message) AS "
                    ...
                    "total_unique_messages, COUNT(*) AS total_messages FROM message_database")
 
    cursor.execute(summary_query)
 
    summary_result = cursor.fetchone()
 
    cursor.execute("SELECT * FROM message_database")
    ...
    cursor.execute("SELECT message FROM message_database")
 
    all_messages = [row[0] for row in cursor.fetchall()]
    cursor.execute("SELECT source FROM message_database")
    ...
    return render_template('db_services.html', summary=summary)
